...
public abstract class TokenCachingStrategy {
    ...
    public static boolean hasTokenInformation(Bundle bundle) {
        if (bundle == null) {
            Log.v("Tag","Coming inside hasTokenInformation 1");
            return false;
        }

        String token = bundle.getString(TOKEN_KEY);
        Log.v("Tag","Coming inside hasTokenInformation 2" + token);
        if ((token == null) || (token.length() == 0)) {
            return false;
        }

        long expiresMilliseconds = bundle.getLong(EXPIRATION_DATE_KEY, 0L);
        Log.v("Tag","Coming inside hasTokenInformation 3" + expiresMilliseconds);
